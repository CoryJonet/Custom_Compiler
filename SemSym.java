...
public class SemSym {
    ...
    public int offset = 0;
    public int scope = -1;
    public static int baseScope = 0;

    // Need to determine if an Id is local or global
    public SemSym(Type type, int scope) {
	this.type = type;
	this.scope = scope;
    }

    public static int getBaseScope() {
        baseScope += 1;
        return baseScope;
    }
...
class FnSym extends SemSym {
    ...
    private int localDeclsSize;

    public FnSym(Type type, int numParams, int size, int scope) {
	super(new FnType(), scope);
	returnType = type;
