...
/**
 * "In addition to implementing the code generator, you will also
 * update the Makefile and the main program (and call it P6.java)..."
 */
...
public class P6 {
    ...
    private PrintWriter codeGenFile;
     ...
     * it
     * @param args command line args array for 
     * [<infile> <outfile> <codegenfile>]
     */
    private P6(String[] args){
	//Parse arguments

	/**
	 * " The code generator should write code to the file named
	 * by the second command-line argument"
	 */
	if (args.length < 2) {
	    String msg = "please supply name of file to be parsed"
		+ " and name of file for code generation.";
	    pukeAndDie(msg);
	}
     ...
     * @param filename path to code generation file
     */
    public void setCodeGenFile(String filename) throws BadOutfileException {
	try {

	    codeGenFile = new PrintWriter(filename);
	...
	/**
	 * "Note that you main program should no longer call the unparser,
	 * nor should it report that the program was parsed successfully."
	 */
	//astRoot.unparse(outFile, 0);

	//"...the code generator is called after the type checker."
	Codegen.p = codeGenFile;
	astRoot.codeGen();
	Codegen.p.close();
